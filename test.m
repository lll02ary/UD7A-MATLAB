...
plot(f_meas/1e3, Zmag_meas, 'k--', 'LineWidth',1.2);
grid on;
xlabel('Frequency (kHz)');
ylabel('|Z| (Ω)');
title('Impedance Magnitude');
legend('補償前','補償後','實測幅值','Location','Best');

% (b) 相位圖
subplot(2,1,2);
plot(f/1e3, angle(Ztotal)*180/pi, 'b-', 'LineWidth',2); hold on;
plot(f/1e3, angle(Z_comp)*180/pi, 'r-', 'LineWidth',2);
plot(f_meas/1e3, Zphase_meas, 'k--', 'LineWidth',1.2);
grid on;
xlabel('Frequency (kHz)');
ylabel('Phase (°)');
title('Impedance Phase');
legend('補償前','補償後','實測相位','Location','Best');

% --- 9. 儲存圖檔 ------------------------------------
print(gcf, 'Z_bode_compare_with_measured', '-dpng', '-r300');
